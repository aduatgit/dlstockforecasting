...
def main():
    ...
    # years = 10
    # start_date = int((datetime.today() - relativedelta(years = years, days=1)).timestamp()) #2 Tage in die Vergangenheit, damit mit dem Input von vor 2 Tagen die letzte Prediction (gestern) gemacht werden kann (1 Tag für lag zwischen x und y; 1 Tag lag von heute (actual) auf gestern)
    # end_date = int((datetime.today() - relativedelta(days=1)).timestamp())

    years = 10
    start_date = (datetime.today() - relativedelta(years=years, days=1)).strftime(
        "%Y-%m-%d"
    )
    ...
    split0, split1, split2 = 0.4, 0.8, 0.9
    ...
    for j in range(0, 5):
        ...
        for i in range(0, int(len(X15) * (1 - split2) + 1)):
            ...
            X_train, X_val, X_test = (
                np.array(
                    X15[
                        int(len(X15) * (split0 + 0.1 * j) + i) : int(
                            len(X15) * (split1 + 0.1 * j) + i
                        )
                    ]
                ),
                np.array(
                    X15[
                        int(len(X15) * (split1 + 0.1 * j) + i) : int(
                            len(X15) * (split2 + 0.1 * j) + i
                        )
                    ]
                ),
                np.array(
                    X15[
                        int(len(X15) * (split2 + 0.1 * j) + i) : int(
                            len(X15) * (split2 + 0.1 * j) + i + 1
                        )
                    ]
                ),
            )
            y_train, y_val, y_test = (
                np.array(
                    y[
                        int(len(y) * (split0 + 0.1 * j) + i) : int(
                            len(y) * (split1 + 0.1 * j) + i
                        )
                    ]
                ),
                np.array(
                    y[
                        int(len(y) * (split1 + 0.1 * j) + i) : int(
                            len(y) * (split2 + 0.1 * j) + i
                        )
                    ]
                ),
                np.array(
                    y[
                        int(len(y) * (split2 + 0.1 * j) + i) : int(
                            len(y) * (split2 + 0.1 * j) + i + 1
                        )
                    ]
                ),
            )
            ...
            prediction_return = (
                prediction - X_test[0][training_variables - 1]
            ) / X_test[0][training_variables - 1]
            stock_return = (y_test - X_test[0][training_variables - 1]) / X_test[0][
                training_variables - 1
            ]
            direction_check = (
                prediction_return / stock_return
            )  # Wenn Positiv gleiche Richtung, wenn negativ falsche Richtung
            ...
            if direction_check >= 0:
                ...
                total_return = total_return * (
                    1 + abs(stock_return)
                )  # return errechnen. hier: wenn die richtung stimmt, dann einfach den return vom tag nehmen
            ...
            print(
                f"Mein total return ist: {np.round((total_return-1)*100, 2)}%, , Stock Total Return: {np.round((stock_total_return-1)*100, 2)}%"
            )
            print(f"MSE: {mse}")
            print(f"Final returns: {total_return_final_list}")
            print(
                f"{np.round(correct_direction / (correct_direction + incorrect_direction)*100, 4)}% der Predictions gingen in die richtige Richtung!"
            )
            print(
                f"Zeit für den Tag: {np.round((time.time() - _start_time)/(i+1),2)} Sekunden"
            )
    ...
    print(
        f"Zeit für den Durchlauf gebraucht: {np.round((time.time() - _start_time)/3600,2)}h"
    )
